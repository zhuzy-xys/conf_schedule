#include "net_qihoo_hconf_Qconf.h"
#include <stdio.h>
#include "hconf.h"
...
static jobject convert_to_java_map(JNIEnv *env, const hconf_batch_nodes *bnodes);
...
(JNIEnv *env, jclass jc, jstring key, jstring idc)
{
    if (NULL == key)
    {
        print_error_message(env, jc, HCONF_ERR_PARAM);
        return NULL;
    }
    ...
    char value_c[HCONF_JAVA_CONF_BUF_MAX_LEN];
    int ret = HCONF_ERR_OTHER;
    if (NULL == idc)
    {
        ret = hconf_get_conf(key_c, value_c, sizeof(value_c), NULL);
    }
    else
    {
        ...
        ret = hconf_get_conf(key_c, value_c, sizeof(value_c), idc_c);
...
(JNIEnv *env, jclass jc, jstring key, jstring idc)
{
    ...
    char host_c[HCONF_HOST_BUF_MAX_LEN] = {0};
    int ret = HCONF_ERR_OTHER;
    if (NULL == idc)
    {
        ret = hconf_get_host(key_c, host_c, sizeof(host_c), NULL);
    }
    else
    {
        const char *idc_c = env->GetStringUTFChars(idc, NULL);
        ...
        ret = hconf_get_host(key_c, host_c, sizeof(host_c), idc_c);
        env->ReleaseStringUTFChars(idc, idc_c);
    }
    env->ReleaseStringUTFChars(key, key_c);
...
(JNIEnv *env, jclass jc, jstring key, jstring idc)
{
    ...
    if (NULL == idc)
    {
        ret = hconf_get_allhost(key_c, &nodes, NULL);
    }
    else
    {
        const char *idc_c = env->GetStringUTFChars(idc, NULL);
        ...
        ret = hconf_get_allhost(key_c, &nodes, idc_c);
        env->ReleaseStringUTFChars(idc, idc_c);
    }
    env->ReleaseStringUTFChars(key, key_c);
    ...
    if (NULL == node_list)
    {   
        print_error_message(env, jc, HCONF_ERR_JAVA_CONVERT_LIST);
        return NULL;
    }   
    return node_list;
}
  ...
  (JNIEnv *env, jclass jc, jstring key, jstring idc)
{
    ...
    int ret = init_hconf_batch_nodes(&bnodes);
    if (HCONF_OK != ret)
    {   
        print_error_message(env, jc, ret);
        env->ReleaseStringUTFChars(key, key_c);
    ...
    if (NULL == idc)
    {
        ret = hconf_get_batch_conf(key_c, &bnodes, NULL);
    }
    else
    {
        const char *idc_c = env->GetStringUTFChars(idc, NULL);
        ...
        ret = hconf_get_batch_conf(key_c, &bnodes, idc_c);
        env->ReleaseStringUTFChars(idc, idc_c);
    }
    env->ReleaseStringUTFChars(key, key_c);
    ...
    if (NULL == node_list)
    {   
        print_error_message(env, jc, HCONF_ERR_JAVA_CONVERT_MAP);
        return NULL;
    }   
    return node_list;
}
  ...
  (JNIEnv *env, jclass jc, jstring key, jstring idc)
{
    ...
    if (NULL == idc)
    {
        ret = hconf_get_batch_keys(key_c, &nodes, NULL);
    }
    else
    {
        const char *idc_c = env->GetStringUTFChars(idc, NULL);
        ...
        ret = hconf_get_batch_keys(key_c, &nodes, idc_c);
        env->ReleaseStringUTFChars(idc, idc_c);
    }
    env->ReleaseStringUTFChars(key, key_c);
...
static int print_error_message(JNIEnv *env, jclass jc, const int error_code)
{
    ...
    {   
        ...
        case HCONF_ERR_HOSTNAME :
            ...
            break;
        case HCONF_ERR_JAVA_CONVERT_LIST:
            message = "Failed to covert string vector to arraylist";
            break;
        case HCONF_ERR_JAVA_CONVERT_MAP:
            message = "Failed to covert hconf_batch_nodes to treeMap";
            break;
        default :
