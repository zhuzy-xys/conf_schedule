 ...
 */
int hconf_get_localidc(qhasharr_t *tbl, std::string &local_idc);

/**
 * Update the tbl using local idc
 */
int hconf_update_localidc(qhasharr_t *tbl, const std::string &local_idc);

/**
 * Check current tblkey whether exist
 */
int hconf_exist_tblkey(qhasharr_t *tbl, const std::string &key, bool &status);
...
int hconf_verify(std::string &val);
