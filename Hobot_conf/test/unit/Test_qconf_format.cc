...
// Unit test case for hconf_format.cc

// Related test environment set up:
class Test_hconf_format : public ::testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(Test_hconf_format, serialize_to_tblkey_invalid_data_type)
{
    int retCode = 0;
    string idc("test"), path("/hconf/demo"), tblkey;

    retCode = serialize_to_tblkey('0', idc, path, tblkey);

    EXPECT_EQ(HCONF_ERR_DATA_TYPE, retCode);
}

// Test for serialize_to_tblkey: data_type=HCONF_DATA_TYPE_ZK_HOST
TEST_F(Test_hconf_format, serialize_to_tblkey_data_type_zk_host)
{
    int retCode = 0;
    string idc("test"), tblkey;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_ZK_HOST, idc, "", tblkey);

    EXPECT_EQ(HCONF_OK, retCode);
    //cout << tblkey << endl;
}

// Test for serialize_to_tblkey: data_type=HCONF_DATA_TYPE_NODE
TEST_F(Test_hconf_format, serialize_to_tblkey_data_type_node)
{
    int retCode = 0;
    string idc("test"), path("/hconf/demo"), tblkey;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_NODE, idc, path, tblkey);

    EXPECT_EQ(HCONF_OK, retCode);
    //cout << tblkey << endl;
}

// Test for serialize_to_tblkey: data_type=HCONF_DATA_TYPE_SERVICE
TEST_F(Test_hconf_format, serialize_to_tblkey_data_type_service)
{
    int retCode = 0;
    string idc("test"), path("/hconf/demo"), tblkey;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_SERVICE, idc, path, tblkey);

    EXPECT_EQ(HCONF_OK, retCode);
    //cout << tblkey << endl;
}

// Test for serialize_to_tblkey: data_type=HCONF_DATA_TYPE_LOCAL_IDC
TEST_F(Test_hconf_format, serialize_to_tblkey_data_type_local_idc)
{
    int retCode = 0;
    string tblkey;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_LOCAL_IDC, "", "", tblkey);
...
TEST_F(Test_hconf_format, deserialize_from_tblkey_invalid_data_type)
{
    int retCode = 0;
    string idc("test"), path("/hconf/demo"), tblkey, out_idc, out_path;
    char data_type = 0;
...
TEST_F(Test_hconf_format, deserialize_from_tblkey_invalid_tblkey_format)
{
    //int retCode = 0;
    string tblkey("2,4,corp11,/hconf/demo33,2,4.corp11./hconf/demo9.demo-test"), idc, path;
    //char data_type;
...
// Test for deserialize_from_tblkey: data_type=HCONF_DATA_TYPE_ZK_HOST
TEST_F(Test_hconf_format, deserialize_from_tblkey_data_type_zk_host)
{
    int retCode = 0;
    string idc, path, tblkey;
    char data_type = 0;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_ZK_HOST, "test", "", tblkey);
    EXPECT_EQ(HCONF_OK, retCode);

    retCode = deserialize_from_tblkey(tblkey, data_type, idc, path);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_EQ(HCONF_DATA_TYPE_ZK_HOST, data_type);
    EXPECT_STREQ("test", idc.data());
}

// Test for deserialize_from_tblkey: data_type=HCONF_DATA_TYPE_NODE
TEST_F(Test_hconf_format, deseralize_from_tblkey_data_type_node)
{
    int retCode = 0;
    string idc, path, tblkey;
    char data_type = 0;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_NODE, "test", "/hconf/demo", tblkey);
    EXPECT_EQ(HCONF_OK, retCode);

    retCode = deserialize_from_tblkey(tblkey, data_type, idc, path);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_EQ(HCONF_DATA_TYPE_NODE, data_type);
    EXPECT_STREQ("test", idc.data());
    EXPECT_STREQ("/hconf/demo", path.data());
}

// Test for deserialize_from_tblkey: data_type=HCONF_DATA_TYPE_SERVICE
TEST_F(Test_hconf_format, deserialize_from_tblkey_data_type_service)
{
    int retCode = 0;
    string idc, path, tblkey;
    char data_type = 0;

    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_SERVICE, "test", "/hconf/demo", tblkey);
    EXPECT_EQ(HCONF_OK, retCode);

    retCode = deserialize_from_tblkey(tblkey, data_type, idc, path);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_EQ(HCONF_DATA_TYPE_SERVICE, data_type);
    EXPECT_STREQ("test", idc.data());
    EXPECT_STREQ("/hconf/demo", path.data());
}

// Test for deserialize_from_tblkey: data_type=HCONF_DATA_TYPE_LOCAL_IDC
TEST_F(Test_hconf_format, deserialize_from_tblkey_data_type_local_idc)
{
    int retCode = 0;
    string idc, path, tblkey;
    char data_type = 0;
    ...
    retCode = deserialize_from_tblkey(tblkey, data_type, idc, path);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_EQ(HCONF_DATA_TYPE_LOCAL_IDC, data_type);
}

// Test for char get_data_type(const string &value)
TEST_F(Test_hconf_format, get_data_type_common)
{
    int retCode = 0;
    string tblkey;
    char data_type = 0;
...
TEST_F(Test_hconf_format, convert_from_localidc_and_tblval)
{
    int retCode = 0;
...
// Test for convert between localidc and tblval
TEST_F(Test_hconf_format, convert_from_localidc_and_tblval_invalid_tblkey)
{
    //int retCode = 0;
    string tblkey, local_idc("test"), tblval, idc_out;
...
TEST_F(Test_hconf_format, convert_from_idcval_and_tblval)
{
    int retCode = 0;
...
TEST_F(Test_hconf_format, convert_from_idcval_and_tblval1)
{
    int retCode = 0;
...
TEST_F(Test_hconf_format, convert_from_nodeval_and_tblval)
{
    int retCode = 0;
...
TEST_F(Test_hconf_format, convert_from_nodeval_and_tblval1)
{
    int retCode = 0;
    ...
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_STREQ("/hconf/demo", path_out.data());
    EXPECT_STREQ("test", idc_out.data());
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_STREQ("demo_value", value_out.data());
}
...
TEST_F(Test_hconf_format, convert_from_chdnodeval_and_tblval)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out;
...
TEST_F(Test_hconf_format, convert_from_chdnodeval_and_tblval1)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out, idc_out, path_out;
    string_vector_t nodes_out;
...
TEST_F(Test_hconf_format, convert_from_batchnodeval_and_tblval)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out, idc_out, path_out;
    string_vector_t nodes_out;
    ...
    retCode = serialize_to_tblkey(HCONF_DATA_TYPE_BATCH_NODE, "test", "/hconf/demo", tblkey);
    EXPECT_EQ(HCONF_OK, retCode);
...
// int batchnodeval_to_tblval(const string &key, const string_vector_t &nodes, string &tblval)
// int tblval_to_batchnodeval(const string &tblval, string_vector_t &nodes)
TEST_F(Test_hconf_format, convert_from_batchnodeval_and_tblval_error_data_type)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out, idc_out, path_out;
    string_vector_t nodes_out;
...
TEST_F(Test_hconf_format, convert_from_batchnodeval_and_tblval1)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out, idc_out, path_out;
    string_vector_t nodes_out;
...
// int batchnodeval_to_tblval(const string &key, const string_vector_t &nodes, string &tblval)
// int tblval_to_batchnodeval(const string &tblval, string_vector_t &nodes, string &idc, string &path)
TEST_F(Test_hconf_format, convert_from_batchnodeval_and_tblval1_error_data_type)
{
    int retCode = 0;
    string tblkey, value("demo_value"), tblval, value_out, idc_out, path_out;
    string_vector_t nodes_out;
...
TEST_F(Test_hconf_format, serialize_to_idc_host)
{
