...
// unit test case for hconf_msg.c

// Related test environment set up and tear down
class Test_hconf_msg : public ::testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(Test_hconf_msg, create_msg_queue_not_exists)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, create_msg_queue_already_exists)
{
    int retCode = 0;
    key_t msg_queue_key = STATIC_MSG_QUEUE_KEY;
...
TEST_F(Test_hconf_msg, create_msg_queue_by_a_new_key)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg,  init_msg_queue_no_exists)
{
    int retCode = 0;

    retCode = init_msg_queue(STATIC_MSG_QUEUE_KEY, msgid);
    
    EXPECT_EQ(HCONF_ERR_MSGGET, retCode);
}

// msg queue exist
TEST_F(Test_hconf_msg,  init_msg_queue_exists)
{
...
TEST_F(Test_hconf_msg, send_msg_unexisted_msgid)
{
    int retCode = 0;
    int msgids = -1;
    ...
    EXPECT_EQ(HCONF_ERR_MSGSND, retCode);
}

// Test for send_msg: send empty message
TEST_F(Test_hconf_msg, send_msg_empty_message)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, send_msg_msg_to_msg_queue_by_msgid_created_by_another_key)
{
    int retCode = 0;
...
// Test for send_msg: length of msg equals with HCONF_MAX_MSG_LEN
TEST_F(Test_hconf_msg, send_msg_length_of_msg_exceed)
{
    int retCode = 0;
    key_t key = STATIC_MSG_QUEUE_KEY;
    create_msg_queue(key, msgid);
    
    string message(HCONF_MAX_MSG_LEN, 'a');
    retCode = send_msg(msgid, message);
    EXPECT_EQ(HCONF_ERR_E2BIG, retCode);
}

// Test for send_msg: length of msg exceed HCONF_MAX_MSG_LEN
TEST_F(Test_hconf_msg, send_msg_length_exceed2)
{
    int retCode = 0;
    key_t key = STATIC_MSG_QUEUE_KEY;
    create_msg_queue(key, msgid);
    
    string message(HCONF_MAX_MSG_LEN + 1, 'a');
    retCode = send_msg(msgid, message);
    EXPECT_EQ(HCONF_ERR_E2BIG, retCode);
}

// Test for send_msg: send msg failed since has no access to write the msg queue
TEST_F(Test_hconf_msg, send_msg_failed_since_has_no_access_to_write)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, send_msg_successfully)
{
    int retCode = 0;
    key_t key = STATIC_MSG_QUEUE_KEY;
    string message("hello");
...
TEST_F(Test_hconf_msg, receive_msg_from_unexisted_msgid)
{
    int retCode = 0;
    string message;

    retCode = receive_msg(-1, message);
    EXPECT_EQ(HCONF_ERR_MSGRCV, retCode);
}

// Test for receive_msg: no message in msg queue
TEST_F(Test_hconf_msg, msg_recv_from_msgid_has_no_msg)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, msg_recv_from_msg_queue_by_msgid_created_by_a_new_key)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, msg_recv_from_msg_queue_but_msg_has_been_received_already)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, receive_msg_successfully)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, receive_msg_times_equals_send_times)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, receive_msg_times_more_than_send_times)
{
    int retCode = 0;
...
TEST_F(Test_hconf_msg, send_msg_repeatedly)
{
    int retCode = 0;
    key_t key = STATIC_MSG_QUEUE_KEY;
    string message("hello");
