...
// unit test case for hconf_shm.cc

// Related test environment set up and tear down

class Test_hconf_shm : public ::testing::Test
{
protected:
...
qhasharr_t *Test_hconf_shm::tbl = NULL;
...
TEST_F(Test_hconf_shm, init_hash_tbl_shm_not_exist)
{
    int retCode = 0;
    ...
    EXPECT_EQ(HCONF_ERR_SHMGET, retCode);
}

// Test for init_hash_tbl: init success
TEST_F(Test_hconf_shm, init_hash_tbl_shm_success)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
    ...
    memsize = qhasharr_calculate_memsize(HCONF_MAX_SLOTS_NUM);
    shmid = shmget(shmkey, memsize, IPC_CREAT | IPC_EXCL | 0666);
    EXPECT_LT(0, shmid);

    retCode = init_hash_tbl(tbl, shmkey, 0666, 0);
    EXPECT_EQ(HCONF_OK, retCode);
    
    EXPECT_EQ(0, shmctl(shmid, IPC_RMID, NULL));
}

// Test for init_hash_tbl: share memory exist but mode larger
TEST_F(Test_hconf_shm, init_hash_tbl_shm_mode_larger)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, init_hash_tbl_shm_mode_smaller)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, init_hash_tbl_shm_mode_no_authentication)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
    ...
    retCode = init_hash_tbl(tbl, shmkey, 0444, 0);
    EXPECT_EQ(HCONF_ERR_SHMAT, retCode);
    
    EXPECT_EQ(0, shmctl(shmid, IPC_RMID, NULL));
}

// Test for init_hash_tbl: share memory exist but only read authentication
TEST_F(Test_hconf_shm, init_hash_tbl_shm_mode_only_read_authentication)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, init_hash_tbl_shm_mode_no_read_authentication)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, create_hash_tbl_common)
{
    int shmid = -1;
    int retCode = 0;
    key_t shmkey = 0x1010ac03;
...
TEST_F(Test_hconf_shm, create_hash_tbl_already_exist)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, create_hash_tbl_already_exist_mode_larger)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, create_hash_tbl_already_exist_mode_smaller)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
TEST_F(Test_hconf_shm, create_hash_tbl_already_exist_no_write_authority)
{
    int shmid = -1;
    size_t memsize = 0;
    int retCode = 0;
...
/**
  *======================================================================================================================
  * Begin_Test_for function: int hconf_get_localidc(qhasharr_t *tbl, string &local_idc)
  */

// Test for get_localidc: tbl NULL
TEST_F(Test_hconf_shm, get_localidc_from_tbl_null)
{
    string localidc;

    int retCode = hconf_get_localidc(NULL, localidc);

    EXPECT_EQ(HCONF_ERR_PARAM, retCode);
}

// Test for get_localidc: tbl->maxslots=0
TEST_F(Test_hconf_shm, get_localidc_from_tbl_zero_maxslots)
{
    int retCode = 0;
    ...
    retCode = hconf_get_localidc(tmp_tbl, localidc);

    EXPECT_EQ(HCONF_ERR_NOT_FOUND, retCode);
...
TEST_F(Test_hconf_shm, get_localidc_from_tbl_key_not_exists)
{
    int retCode = 0;
    string localidc;

    retCode = hconf_get_localidc(tbl, localidc);

    EXPECT_EQ(HCONF_ERR_NOT_FOUND, retCode);
}

// Test for get_localidc: key exists in val
TEST_F(Test_hconf_shm, get_localidc_from_tbl_key_exists_in_val)
{
    int retCode = 0;
    string tblkey, tblval, localidc;
    serialize_to_tblkey(HCONF_DATA_TYPE_LOCAL_IDC, "", "", tblkey);
...
/**
  * End_Test_for function: int hconf_get_localidc(qhasharr_t *tbl, string &local_idc)
  *==============================================================================================================
  */
...
/**
  *======================================================================================================
  * Begin_Test_for function: int hconf_update_localidc(qhasharr_t *tbl, const string &local_idc)
  */

// Test for hconf_update_localidc: tbl null
TEST_F(Test_hconf_shm, hconf_update_localidc_tbl_null)
{
    int retCode = 0;

    retCode = hconf_update_localidc(NULL, "test");

    EXPECT_EQ(HCONF_ERR_PARAM, retCode);
}

// Test for hconf_update_localidc: tbl->maxslots=0
TEST_F(Test_hconf_shm, hconf_update_localidc_zero_maxslots)
{
    int retCode = 0;
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
    ...
    retCode = hconf_update_localidc(tmp_tbl, "test");

    EXPECT_EQ(HCONF_ERR_TBL_SET, retCode);
...
// Test for hconf_update_localidc: tbl->maxslots > 0
TEST_F(Test_hconf_shm, hconf_update_localidc_no_zero_maxslots)
{
    int retCode = 0;
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
...
// Test for hconf_update_localidc: already exists and update by the same value
TEST_F(Test_hconf_shm, hconf_update_localidc_update_by_the_same_value)
{
    int retCode = 0;
    retCode = hconf_update_localidc(tbl, "test");
    EXPECT_EQ(HCONF_OK, retCode);
...
// Test for hconf_update_localidc: already exists and update by different value
TEST_F(Test_hconf_shm, hconf_update_localidc_update_by_different_value)
{
    int retCode = 0;
    retCode = hconf_update_localidc(tbl, "test");
    EXPECT_EQ(HCONF_OK, retCode);
...
// Test for hconf_update_localidc: empty localidc
TEST_F(Test_hconf_shm, hconf_update_localidc_update_empty)
{
    int retCode = 0;
    string localidc;
    retCode = hconf_update_localidc(tbl, "");
    EXPECT_EQ(HCONF_ERR_PARAM, retCode);
}
/**
  * End_Test_for function: int hconf_update_localidc(qhasharr_t *tbl, const string &local_idc)
  *================================================================================================
  */

/**
  * Begin_Test_for function: int hconf_exist_tblkey(qhasharr_t *tbl, const string &key, bool &status)
  *================================================================================================
  */

// Test for hconf_exist_tblkey: tbl null
TEST_F(Test_hconf_shm, hconf_exist_tblkey_tbl_null)
{
    int retCode = 0;
    ...
    retCode = hconf_exist_tblkey(NULL, tblkey, status);
    EXPECT_EQ(HCONF_ERR_PARAM, retCode);
}

// Test for hconf_exist_tblkey: tblkey empty
TEST_F(Test_hconf_shm, hconf_exist_tblkey_tblkey_empty)
{
    int retCode = 0;
    bool status = false;

    retCode = hconf_exist_tblkey(tbl, "", status);

    EXPECT_EQ(HCONF_ERR_PARAM, retCode);
}

// Test for hconf_update_localidc: tbl->maxslots=0
TEST_F(Test_hconf_shm, hconf_exist_tblkey_err_tbl)
{
    int retCode = 0;
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
    ...
    retCode = hconf_exist_tblkey(tmp_tbl, tblkey, status);
...
// Test for hconf_exist_tblkey: not exist
TEST_F(Test_hconf_shm, hconf_exist_tblkey_not_exist)
{
    int retCode = 0;
    string tblkey, tblval;
    bool status = false;
    ...
    retCode = hconf_exist_tblkey(tbl, tblkey, status);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_EQ(false, status);
}

// Test for hconf_exist_tblkey: exist
TEST_F(Test_hconf_shm, hconf_exist_tblkey_exist)
{
    int retCode = 0;
    string tblkey, tblval;
    bool status = false;
...
/**
  * End_Test_for function: int hconf_exist_tblkey(qhasharr_t *tbl, const string &key, bool &status)
  *================================================================================================
  */
...
TEST_F(Test_hconf_shm, hash_tbl_get_count_tbl_null)
{
...
TEST_F(Test_hconf_shm, hash_tbl_get_count_tmp_tbl_common)
{
    int max_slots = -1, used_slots = -1, retCode = -1;
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
...
// Test for hash_get_count: success
TEST_F(Test_hconf_shm, hash_tbl_get_count_tbl_common)
{
    int max_slots = -1, used_slots = -1, retCode = -1;
...
TEST_F(Test_hconf_shm, hash_tbl_get_count_tbl_has_item)
{
    int max_slots = -1, used_slots = -1, retCode = -1;
...
TEST_F(Test_hconf_shm, hash_tbl_set_zero_maxslots)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_tbl_null)
{
    int retCode = 0;
    string key("hello"), val("world");
...
TEST_F(Test_hconf_shm, hash_tbl_set_key_empty)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_key_not_exists_in_tbl)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_with_same_value)
{
    int retCode = 0;
    string key("hello"), val("world"), val_out;
    ...
    retCode = hash_tbl_set(tbl, key, val);
    EXPECT_EQ(HCONF_ERR_SAME_VALUE, retCode);
...
TEST_F(Test_hconf_shm, hash_tbl_set_key_set_with_new_value)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_slots_needed_of_new_value_less_than_old_value)
{
    int retCode = 0;
...
// Test for hash_tbl_set: old value needs 2 slots but new value need 1 slot
TEST_F(Test_hconf_shm, hash_tbl_set_slots_needed_of_new_value_more_than_old_value)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_usedslots_equals_with_maxslots)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_remain_slots_not_enough)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_set_replace_remain_slots_not_enough)
{
    int retCode = 0;
    string keys[] = {"abc", "def", "ghi", "jkl"};
    string vals[]= {"123", "456", "789", "123"};
...
TEST_F(Test_hconf_shm, hash_tbl_set_replace_remain_slots_enough)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_zero_maxslots)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_tbl_null)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_tbl_empty_key)
{
    int retCode = 0;
    string key, val;
...
TEST_F(Test_hconf_shm, hash_tbl_get_key_not_exists)
{
    int retCode = 0;
    string key("hello"), val;
...
TEST_F(Test_hconf_shm, hash_tbl_get)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_key_truncated)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_exists_and_key_is_truncated_get_by_truncated_key)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_get_key_exists_and_value_needs_two_slots)
{
    int retCode = 0;
...
/**
  * Begin_Test_for function: int hconf_verify(string &val)
  *==============================================================================================================
  */
// Test for hconf_verify: success
TEST_F(Test_hconf_shm, hconf_verify_common)
{
    int retCode = 0;
    char val_md5[HCONF_MD5_INT_LEN] = {0};
    string val("hello");
    qhashmd5(val.data(), val.size(), val_md5);
    val.append(val_md5, HCONF_MD5_INT_LEN);

    retCode = hconf_verify(val);
    EXPECT_EQ(HCONF_OK, retCode);
    EXPECT_STREQ("hello", val.data());
}

// Test for hconf_verify: md5 error
TEST_F(Test_hconf_shm, hconf_verify_error_md5)
{
    int retCode = 0;
    char val_md5[HCONF_MD5_INT_LEN] = {0};
    string val("hello");
    ...
    retCode = hconf_verify(val);
    EXPECT_EQ(HCONF_ERR_TBL_DATA_MESS, retCode);
}

// Test for hconf_verify: empty
TEST_F(Test_hconf_shm, hconf_verify_val_empty)
{
    int retCode = 0;
    string val;
...
/**
  * End_Test_for function: int hconf_verify(string &val)
  *==============================================================================================================
  */
...
TEST_F(Test_hconf_shm, hash_tbl_exist_tbl_null)
{
    int retCode = 0;
    string tblkey, tblval;
    bool status = false;
...
TEST_F(Test_hconf_shm, hash_tbl_exist_tblkey_empty)
{
    bool status = false;
...
TEST_F(Test_hconf_shm, hash_tbl_exist_err_tbl)
{
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
    char* memory = NULL;
...
TEST_F(Test_hconf_shm, hash_tbl_exist_not_exist)
{
    string tblkey, tblval;
    bool status = false;
...
TEST_F(Test_hconf_shm, hash_tbl_exist_exist)
{
    int retCode = 0;
    string tblkey, tblval;
    bool status = false;
...
TEST_F(Test_hconf_shm, hash_tbl_remove_null)
{
    int retCode = 0;
    string tblkey("hello");
...
// Test for hash_tbl_exist: tblkey empty
TEST_F(Test_hconf_shm, hash_tbl_remove_tblkey_empty)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_remove_maxslots_zero)
{
    int retCode = 0;
    qhasharr_t* tmp_tbl = NULL;
    int memsize = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_remove_key_not_exists)
{
    int retCode = 0;
    string key("hello");
...
TEST_F(Test_hconf_shm, hash_tbl_remove_key_exists)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_remove_all_key_exists)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_clear_null)
{
    int retCode = 0;
...
TEST_F(Test_hconf_shm, hash_tbl_clear_all)
{
    int retCode = 0;
    string keys[] = {"abc", "def", "hijk", "lmn", "opq"};
    string vals[] = {"1234", "111", "aaaa", "bbb", "1"};
...
TEST_F(Test_hconf_shm, hash_tbl_getnext_tbl_null)
{
    int retCode = 0, idx = -1;
...
TEST_F(Test_hconf_shm, hash_tbl_getnext_tbl_empty)
{
    int retCode = 0, idx = 0;
    string tblkey_out, tblval_out;
    EXPECT_EQ(0, tbl->usedslots);
    ...
    EXPECT_EQ(HCONF_ERR_TBL_END, retCode);
}

// Test for hash_tbl_getnext: tbl->maxslots=0
TEST_F(Test_hconf_shm, hash_tbl_getnext_zero_maxslots)
{
    int retCode = 0;
    string key, val;
    qhasharr_t* tmp_tbl = NULL;
...
TEST_F(Test_hconf_shm, hash_tbl_getnext_type_node)
{
    int retCode = 0;
    ...
    {
        string tblkey, tblval;
        serialize_to_tblkey(HCONF_DATA_TYPE_NODE, idcs[i], paths[i], tblkey);
...
TEST_F(Test_hconf_shm, hash_tbl_getnext_type_mix)
{
    int retCode = 0;
    string tblkey, tblval;

    //node
    serialize_to_tblkey(HCONF_DATA_TYPE_NODE, "abc", "/demo/1", tblkey);
    ...
    //service
    serialize_to_tblkey(HCONF_DATA_TYPE_SERVICE, "def", "/demo/2", tblkey);
    ...
    //batch node
    serialize_to_tblkey(HCONF_DATA_TYPE_BATCH_NODE, "ghi", "/demo/3", tblkey);
    ...
    //zk_host
    serialize_to_tblkey(HCONF_DATA_TYPE_ZK_HOST, "jkl", "", tblkey);
